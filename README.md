...
Yuragi 是一款运行在浏览器中的**拙趣感手绘字生成器**。

它做的不是又一套「手写体」字体。常见的手写字体每个字的形状是固定的，同一个字排十次都一模一样；Yuragi 追求的是近年茶饮海报（比如喜茶）里那种拙趣的手绘字：笔画不那么规矩，字有大有小、微微歪斜，同一个字写两次也不一样，像真有人拿笔一个字一个字写下来。

做法是只画干净的字形骨架，「拙」交给算法：沿笔画叠加低频抖动、收笔轻轻越位，排成一行时逐字摇大小和倾斜，重复出现的字每次都重写一遍骨架。绘制骨架后可以调整手写变化、预览字形排列，并导出 SVG、PNG 或书写过程动画。同一工程和种子会生成相同结果，满意的那一版随时能复现。
...
可以让 Claude Code、Codex 等在对话里直接写出同样拙趣的手绘字 SVG。它同时是上面回归对照的基准实现，
