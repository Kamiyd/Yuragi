...
1. **渲参照对着看，不凭记忆写。** 黑体核结构（笔画数、笔顺、部件位置），小赖定比例（重心、笔长、呼应）；
   两者打架听小赖，除非会读成别的字。参照只看不描，字体的装饰和上扬的横不进骨架。
   ```bash
...
3. **先摆结构再调细节**：左右结构先定分界线，上下结构先定横向分界。
4. **写进 JSON**，新字加在字库**末尾**：
   ```json
   {"vb": 64, "sw": 2.8, "items": {
     "今": [
   ...
   ```
   汉字**每一笔都写 `"amp": 0.65`**；转折在路径里自带半径（横折 `Q`，r≈2.5），钩要往回上方甩。
