...
_cr 里还有两道「不许自交」的闸，只保证路径不打结，不改手感：
...
def _ht(anchor, handle, other):
    """手柄投影到弦上的比例：1 表示正好落在对端。"""
    vx, vy = other[0] - anchor[0], other[1] - anchor[1]
    L2 = vx*vx + vy*vy
    if L2 == 0: return 0
