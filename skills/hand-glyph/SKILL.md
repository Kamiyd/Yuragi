...
部件一大一小、框小而方、折角尖、钩短，**字有大有小、宽窄随字** —— 不是每个字都撑成一样大的方块
（规矩见 `reference/glyphs.md`「骨架要拙」「字有大有小」）。
主干的交接照样接好 —— 喜茶的字**一处都不出头**，框的角都是封死的；
字库表头写 `"over": 0`，关掉滤镜的收笔越位，不然 T 字交接会支出一小截；
再写 `"fit": 12`，按字宽排 —— 字有大有小之后，等宽格子会让小字两边空一大块；
`"drift": 1` 让字上下错落、字距忽近忽远、多行缩进不一样；`"amp": 0.65` 让线更直。
...
规格：`viewBox` 64 网格一字一格；字面不出 7–57，**但每个字画多大、多宽随字定**；线宽 `sw` 汉字 **2.8**、拉丁 **3.4** 是**字库的基础线宽**；
...
- 中文一行 + 英文一行用 `compose`：字面框居中、两行线宽自动补偿、透明输出；`--main latin` 让英文做主标题，
  英文做副标题时加 `--sub-weight 0.8`（一样粗时英文显重），
...
1. **渲参照只核结构，不抄样子。** 黑体只用来数笔画、对笔顺、看部件在哪。
   **比例、笔长、框的大小都别照参照** —— 它是规整的字体，照着写就又是印刷体。
   参照只看不描。
   ```bash
...
3. **先定这个字画多大、多宽**：笔画多的大、笔画少的小，窄字画窄、扁字画扁（`glyphs.md`「字有大有小」那张表）。
   **再摆结构、调细节**：左右结构先定分界线，上下结构先定横向分界 —— 分界别放在正中，
   让两块部件一大一小。然后按 `glyphs.md`「骨架要拙」让点和小部件浮起来、折角写尖、钩写短。
4. **写进 JSON**，新字加在字库**末尾**：
   ```json
   {"vb": 64, "sw": 2.8, "amp": 0.65, "over": 0, "fit": 12, "drift": 1, "items": {
     "今": [
   ...
   拉丁字母在第一笔写 `adv`，`adv = 字面宽 + 5`（左右各留 2.5）。英文字母**骨架照规整的写法画**，
   画完用 `handdraw.py latin-zhuo` 整体变拙（见 `glyphs.md`「英文怎么变拙」），别在骨架里手工画歪。
...
| 命令 | 做什么 |
|---|---|
| `handdraw.py doctor` | 检查 Pillow、参照字体、编辑器 |
